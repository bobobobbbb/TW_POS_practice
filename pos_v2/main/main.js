...
const itemJsObj = require('./model/item')
const promotionJsObj = require('./model/promotion')

const loadAllItems = itemJsObj.Item.all()
const loadAllPromotions = promotionJsObj.Promotion.all()
...
function showTable(inputs) {
    ...
    var date = new Date()
    console.log(`打印时间：${date.getFullYear()}年${date.getMonth()}月${date.getDate()}日 ${date.toString().slice(16,24)}`)
    var totalPrice = 0
    var saveMoney = 0
    for (let i = 0; i < inputs.length; i++) {
